...
【モード】 -m / --mode
  ...
  commits    特定の2コミット間                          … git diff <from> <to>
             別名: range    ※ -f / -t で指定
  select     ブランチと2コミットを対話選択して比較      … ブランチ選択・ページャー付き
             別名: pick, interactive
...
【使用例】
  ...
  # ブランチと2コミットを対話的に選んで差分を表示 (ページャー付き)
...
select      pick, interactive   ブランチと2コミットを対話選択 (ページャー付き)
...
# --- 1) ブランチの対話選択 (既定値: main) ---
select_branch_interactive() {
  local default_branch="${FROM:-main}"
  local branches=()
  local cur_branch
  ...
  local B="" R="" CYN="" YEL="" GRN="" GRY=""
  if [[ "$USE_COLOR" == "yes" ]]; then
    B=$'\033[1m'; R=$'\033[0m'; CYN=$'\033[36m'; YEL=$'\033[33m'; GRN=$'\033[32m'; GRY=$'\033[90m'
  fi

  echo
  printf '%s================================================================================%s\n' "$CYN" "$R"
  printf ' %sブランチの選択%s (コミット差分を比較するブランチを選択してください)\n' "$B" "$R"
  printf '%s================================================================================%s\n' "$CYN" "$R"

  if ((${#branches[@]} > 0)); then
    printf ' %s[利用可能なローカルブランチ]%s\n' "$YEL" "$R"
    local i b_name marker
    for i in "${!branches[@]}"; do
      b_name="${branches[i]}"
      marker=""
      if [[ "$b_name" == "$cur_branch" ]]; then
        marker=" ${GRN}(現在のブランチ)${R}"
      fi
      if [[ "$b_name" == "$default_branch" ]]; then
        marker+=" ${CYN}[既定]${R}"
      fi
      printf '   %s%2d)%s %s%s\n' "$B" "$((i + 1))" "$R" "$b_name" "$marker"
    done
    echo
  fi
  printf ' %s※ リモートブランチ (origin/main 等) や任意のブランチ名も直接入力可能です。%s\n' "$GRY" "$R"
  printf ' %s※ [q] で終了します。%s\n' "$GRY" "$R"
  echo

  local input chosen=""
  while true; do
    printf '対象ブランチを入力または番号で選択してください [既定: %s%s%s]: ' "$B" "$default_branch" "$R"
    ...
    if [[ -z "$input" ]]; then
      ...
      info "処理をキャンセルしました。"
      exit 0
    elif [[ "$input" =~ ^[0-9]+$ ]] && ((input >= 1 && input <= ${#branches[@]})); then
      chosen="${branches[$((input - 1))]}"
    else
      chosen="$input"
    fi

    if "${GIT[@]}" rev-parse --verify --quiet "${chosen}^{commit}" >/dev/null 2>&1; then
      SELECTED_BRANCH="$chosen"
      printf '\n %sブランチ%s: %s%s%s を選択しました。\n' "$B" "$R" "$GRN" "$SELECTED_BRANCH" "$R"
      return 0
    else
      err "ブランチ '$chosen' が解決できません。存在するブランチ名または番号を指定してください。"
      if [[ "$chosen" == "main" ]] && "${GIT[@]}" rev-parse --verify --quiet "master^{commit}" >/dev/null 2>&1; then
        info "ヒント: このリポジトリには 'master' ブランチが存在します。"
      fi
    fi
  done
}
...
load_branch_commits() {
  ...
  local us=$'\037' h d a s
  while IFS="$us" read -r h d a s; do
    [[ -n "$h" ]] || continue
    COMMITS_HASH+=("$h")
    COMMITS_DATE+=("$d")
    COMMITS_AUTHOR+=("$a")
    COMMITS_SUBJ+=("$s")
  done < <("${GIT[@]}" log "$target" --date=format:'%Y-%m-%d %H:%M:%S' --pretty=format:"%h${us}%ad${us}%an${us}%s" 2>/dev/null)

  TOTAL_COMMITS="${#COMMITS_HASH[@]}"
  if ((TOTAL_COMMITS == 0)); then
    die "ブランチ '$target' にコミットがありません。"
  elif ((TOTAL_COMMITS < 2)); then
    die "ブランチ '$target' にはコミットが 1 件しかありません。2点間のコミット差分を表示するには最低 2 件のコミットが必要です。"
  fi
  return 0
}

# --- 3) コミット一覧のページ表示 (1ページ10件) ---
display_commit_page() {
  local page="$1"
  local from_idx="${2:--1}"
  local total_pages=$(( (TOTAL_COMMITS + PAGE_SIZE - 1) / PAGE_SIZE ))
  local start_idx=$(( (page - 1) * PAGE_SIZE ))
  local end_idx=$(( start_idx + PAGE_SIZE - 1 ))
  ((end_idx >= TOTAL_COMMITS)) && end_idx=$((TOTAL_COMMITS - 1))

  local B="" R="" CYN="" YEL="" GRN="" GRY="" MAG=""
  if [[ "$USE_COLOR" == "yes" ]]; then
    B=$'\033[1m'; R=$'\033[0m'; CYN=$'\033[36m'; YEL=$'\033[33m'; GRN=$'\033[32m'
    GRY=$'\033[90m'; MAG=$'\033[35m'
  fi

  echo
  printf '%s================================================================================%s\n' "$CYN" "$R"
  printf ' %sコミット一覧%s [ブランチ: %s%s%s] (%sページ %d/%d%s / %d-%d件目 / 全%d件)\n' \
    "$B" "$R" "$GRN" "$SELECTED_BRANCH" "$R" "$YEL" "$page" "$total_pages" "$R" \
    "$((start_idx + 1))" "$((end_idx + 1))" "$TOTAL_COMMITS"
  printf '%s================================================================================%s\n' "$CYN" "$R"
  printf '  %sNo.   コミット  コミット日時         作成者          件名%s\n' "$B" "$R"
  printf '  %s------------------------------------------------------------------------------%s\n' "$GRY" "$R"

  local idx no h d a s mark line_col
  for ((idx = start_idx; idx <= end_idx; idx++)); do
    no=$((idx + 1))
    h="${COMMITS_HASH[idx]}"
    d="${COMMITS_DATE[idx]}"
    a="${COMMITS_AUTHOR[idx]}"
    s="${COMMITS_SUBJ[idx]}"

    if ((${#a} > 14)); then a="${a:0:13}…"; fi
    ...
    printf '%s%s[%3d]  %s%-7s%s  %s%s%s  %-14s  %s%s%s\n' \
      "$line_col" "$mark" "$no" "$MAG" "$h" "$R" "$GRY" "$d" "$R" "$a" "$line_col" "$s" "$R"
  done
  printf '  %s------------------------------------------------------------------------------%s\n' "$GRY" "$R"

  if ((from_idx >= 0)); then
    printf '  %s選択済 [比較元 (From)]: [%3d] %s (%s - %s)%s\n' \
      "$GRN" "$((from_idx + 1))" "${COMMITS_HASH[from_idx]}" "${COMMITS_DATE[from_idx]}" "${COMMITS_SUBJ[from_idx]}" "$R"
    printf '  %s------------------------------------------------------------------------------%s\n' "$GRY" "$R"
  fi

  printf '  %s操作: [n] 次ページ  [p] 前ページ  [番号] 選択  [q] 終了%s\n' "$YEL" "$R"
  echo
}

# --- 4) 2点間のコミットを対話選択 ---
select_two_commits_interactive() {
  local cur_page=1
  local total_pages=$(( (TOTAL_COMMITS + PAGE_SIZE - 1) / PAGE_SIZE ))
  local from_idx=-1
  local to_idx=-1
  ...
  # Step 1: 比較元 (From: 変更前) の選択
  while true; do
    display_commit_page "$cur_page" "-1"
    printf '比較元 (From / 変更前) のコミット番号を入力してください [1-%d, n/p:ページ切替, q:終了]: ' "$TOTAL_COMMITS"
    if ! IFS= read -r input; then
      echo
      die "入力を読み取れませんでした。"
    fi
    ...
    case "$input" in
      ...
      n|next|f)
        if ((cur_page < total_pages)); then
          ...
          warn "すでに最後のページです (ページ $cur_page / $total_pages)。"
        fi
        ;;
      p|prev)
        if ((cur_page > 1)); then
          ...
          warn "すでに先頭のページです (ページ $cur_page / $total_pages)。"
        fi
        ;;
      "")
        warn "番号を入力してください。"
      ...
      *)
        if [[ "$input" =~ ^([0-9]+)[[:space:]]+([0-9]+)$ ]]; then
          ...
          if ((n1 >= 1 && n1 <= TOTAL_COMMITS && n2 >= 1 && n2 <= TOTAL_COMMITS)); then
            if ((n1 == n2)); then
              err "比較元と比較先に同じコミット番号 ($n1) が指定されています。異なる番号を指定してください。"
            else
              from_idx=$((n1 - 1))
              to_idx=$((n2 - 1))
              break
            ...
            err "指定番号が範囲外です (1〜$TOTAL_COMMITS の範囲で指定してください)。"
          fi
        elif [[ "$input" =~ ^[0-9]+$ ]]; then
          local n="$input"
          if ((n >= 1 && n <= TOTAL_COMMITS)); then
            from_idx=$((n - 1))
            break
          else
            err "指定番号が範囲外です (1〜$TOTAL_COMMITS の範囲で指定してください)。"
          ...
          err "無効な入力です: '$input' (番号、n、p、q のいずれかを入力してください)"
        fi
        ;;
    esac
  done

  # Step 2: 比較先 (To: 変更後) の選択 (ショートカットで未選択の場合)
  if ((to_idx < 0)); then
    while true; do
      display_commit_page "$cur_page" "$from_idx"
      printf '比較先 (To / 変更後) のコミット番号を入力してください [既定: 1(最新), 1-%d, n/p:ページ切替, b:戻る, q:終了]: ' "$TOTAL_COMMITS"
      if ! IFS= read -r input; then
        echo
        die "入力を読み取れませんでした。"
      fi
      ...
      case "$input" in
        ...
        *)
          if [[ "$input" =~ ^[0-9]+$ ]]; then
            ...
            if ((n >= 1 && n <= TOTAL_COMMITS)); then
              local cand_to=$((n - 1))
              if ((cand_to == from_idx)); then
                err "比較元と同じコミット番号 ($n) です。異なるコミットを選択してください。"
              else
                to_idx=$cand_to
                break
              fi
            ...
            err "無効な入力です: '$input' (番号、n、p、b、q のいずれかを入力してください)"
          fi
          ;;
      esac
    done
  ...
  CHOSEN_FROM_COMMIT="${COMMITS_HASH[from_idx]}"
  CHOSEN_TO_COMMIT="${COMMITS_HASH[to_idx]}"

  if ((from_idx < to_idx)); then
    info "注: 比較元の方が比較先よりも新しいコミットです (逆順差分となります)。"
  fi

  local B="" R="" CYN="" GRN="" YEL=""
  if [[ "$USE_COLOR" == "yes" ]]; then
    B=$'\033[1m'; R=$'\033[0m'; CYN=$'\033[36m'; GRN=$'\033[32m'; YEL=$'\033[33m'
  fi

  echo
  printf '%s================================================================================%s\n' "$CYN" "$R"
  printf ' %s差分表示の対象コミットが決定しました:%s\n' "$B" "$R"
  printf '   %s比較元 (From / 変更前)%s : [%3d] %s (%s  %s)\n' \
    "$YEL" "$R" "$((from_idx + 1))" "$CHOSEN_FROM_COMMIT" "${COMMITS_DATE[from_idx]}" "${COMMITS_SUBJ[from_idx]}"
  printf '   %s比較先 (To   / 変更後)%s : [%3d] %s (%s  %s)\n' \
    "$GRN" "$R" "$((to_idx + 1))" "$CHOSEN_TO_COMMIT" "${COMMITS_DATE[to_idx]}" "${COMMITS_SUBJ[to_idx]}"
  printf '%s================================================================================%s\n' "$CYN" "$R"
  echo
...
run_interactive_selection() {
  select_branch_interactive || return 1
  load_branch_commits "$SELECTED_BRANCH" || return 1
  select_two_commits_interactive || return 1

  FROM="$CHOSEN_FROM_COMMIT"
  TO="$CHOSEN_TO_COMMIT"
  return 0
}
...
resolve_mode() {
  ...
  case "$MODE" in
    ...
    select|pick|interactive|choose|commits-interactive) MODE="select" ;;
  ...
  case "$MODE" in
    ...
    commits)
      if [[ -z "$FROM" ]]; then
        if [[ -t 0 ]] || ((DO_INTERACTIVE)); then
          info "比較元コミットが未指定のため、対話選択モードを起動します。"
          run_interactive_selection || exit 1
        else
          die "commits モードでは -f <コミット> を指定してください。"
        fi
      fi
    ...
    select)
      ...
      verify_ref "$TO"   || die "比較先コミットが解決できません: $TO"
      if [[ "$MERGE_BASE" == "yes" ]]; then
        RANGE=("${FROM}...${TO}"); MODE_DESC="${SELECTED_BRANCH}: ${FROM} と ${TO} の共通祖先 ⇔ ${TO} (三点比較)"
      else
        RANGE=("$FROM" "$TO");     MODE_DESC="${SELECTED_BRANCH}: ${FROM} ⇔ ${TO} (二点比較)"
      fi
      LOG_RANGE="${FROM}..${TO}"
      SIDE_L="$FROM ($("${GIT[@]}" rev-parse --short "$FROM"))"
      SIDE_R="$TO ($("${GIT[@]}" rev-parse --short "$TO"))"
      MODE_NOTE="ブランチ '${SELECTED_BRANCH}' で選択された2つのコミット間の変更を確認します。"
      ;;
    branches)
...
build_diff_opts() {
  ...
  ((${#PATHSPEC[@]})) && DIFF_CMD_DISPLAY+=" -- ${PATHSPEC[*]}"
}
...
9 種類の差分に対応|ワークツリー / ステージ / HEAD / 直近 N コミット / 特定コミット / 2 コミット間 / コミット対話選択 / ブランチ間 / リモート追跡ブランチ を -m や -i の指定で切り替えられます。
...
select|pick, interactive|選択コミット (From)|選択コミット (To)|git diff <from> <to>|ブランチを選択し、コミット一覧をページ送りしながら2点間を選びたいとき
...
EOS
  ...
  man_tip "「ブランチを選んでコミット一覧を見ながら2点間を比べたい」  →  -m select または -i"
...
対話操作|-i|--interactive|(なし)|無効|コミット対話選択モードを起動。ブランチ選択とページャーで2点間を選択します。
...
8|ブランチとコミットを選んで差分を見る|./git-diff-helper.sh -m select|対話形式でページ送りしながら2点間を選択。-i でも可。
9|ブランチのレビュー用に差分を出す|./git-diff-helper.sh -m branches -f main -t feature/login|分岐後の変更のみ (三点比較)。
10|ブランチの先端同士を単純比較する|./git-diff-helper.sh -m branches -f main -t feature/login --no-merge-base|両者の現在の状態を比べます。
11|push 前にリモートとの差を見る|./git-diff-helper.sh -m remote|追跡ブランチを自動判定します。
12|比較先のリモートを明示する|./git-diff-helper.sh -m remote -t origin/develop|追跡設定が無い場合はこちら。
13|対象ディレクトリを絞り込む|./git-diff-helper.sh -m head -- src/ docs/README.md|パス指定は -- の後に書きます。
14|別のリポジトリを対象にする|./git-diff-helper.sh -m head -r /srv/git/myapp|cd は不要です。
15|出力先を変える|./git-diff-helper.sh -m head -o /var/tmp/diffrep|存在しない場合は自動作成します。
16|ファイル名の接頭辞を変える|./git-diff-helper.sh -m head --prefix release-1.1|案件ごとに整理できます。
17|Markdown だけを作る|./git-diff-helper.sh -m head --no-excel --no-text --no-screen|課題管理システムへの貼り付け用。
18|Markdown に行番号を併記する|./git-diff-helper.sh -m head --md-linenos|差分行の先頭に旧 / 新の行番号を表示します。
19|空白だけの差分を無視する|./git-diff-helper.sh -m head -w|インデント修正時に有効です。
20|サマリだけを素早く見る|./git-diff-helper.sh -m head --summary-only|大量差分の全体把握に。
21|巨大な差分を扱う|./git-diff-helper.sh -m branches -f main -t big --max-lines 200 --excel-max-rows 50000|表示行数と Excel 行数を制限します。
22|文脈行を増やして読む|./git-diff-helper.sh -m head -U 10|変更前後を 10 行ずつ表示します。
23|バッチ処理でファイルだけ作る|./git-diff-helper.sh -m head --no-screen -o /var/log/diff|cron などでの定期取得に。
24|色付きのまま画面を送る|./git-diff-helper.sh -m head --pager|less -R に流します。
25|利用ガイドだけを作る|./git-diff-helper.sh --manual-only -o ./docs|本ファイルを再生成します。
EOS
  MAN_Z=0
  man_filter
...
コミット履歴|No / コミット / 日時 / 作成者 / 件名|対象範囲のコミット一覧。|prev / commit / commits / branches / remote モードで出力されます。
EOS
  MAN_Z=0
...
[ 対象コミット一覧 ]|prev / commit / commits / branches / remote モードでのみ表示されます。
...
EOS
  ...
  man_section "終了コード"
  man_head "コード" "意味" "補足"
  man_row "" center 0 plain "正常終了" plain "差分の有無にかかわらず 0 を返します。差分の有無で分岐したい場合は出力ファイルの内容をご確認ください。"
